...
- Investigate and comparate the behaviour of previously mentioned MOG theories with both :math:`\Lambda`CDM and :math:`\nu`CDM theories. Constrain the parameter space for all three theories using MCMC (Markov Chain Monte Carlo) approach, study Hubble and :math:`\sigma_8` tensions, matter power spectrum :math:`P(k)` and bispectrum :math:`B(k)`, 2PCF/3PCF and halo mass functions.
